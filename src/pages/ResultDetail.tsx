...
export default function ResultDetail() {
  ...
  const test = submission ? tests.find((t) => t.id === submission.test_id) : null;

  const [editedResults, setEditedResults] = useState<EditableResult[]>([]);
  const [isSaving, setIsSaving] = useState(false);

  useEffect(() => {
    if (test?.questions && test.questions.length > 0) {
      setEditedResults(test.questions.map((q) => ({
        questionId: q.id,
        extractedAnswer: "Sample extracted answer",
        modelAnswer: q.correct_answer,
        marksAwarded: Math.floor(Math.random() * q.marks),
        maxMarks: q.marks,
        aiExplanation: "AI evaluation pending",
        ocrConfidence: 0.9,
      })));
    } else if (submission) {
      setEditedResults([{
        questionId: "1",
        extractedAnswer: "Answer extracted from submission",
        modelAnswer: "Model answer",
        marksAwarded: submission.marks_obtained || 0,
        maxMarks: submission.max_marks || 100,
        aiExplanation: "AI grading complete",
        ocrConfidence: 0.95,
      }]);
    }
  }, [test, submission]);
            ...
            <Card key={result.questionId}>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline">{result.marksAwarded}/{result.maxMarks} marks</Badge>
                </div>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="space-y-4">
                    ...
                    {result.ocrConfidence !== undefined && (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-medium">{Math.round(result.ocrConfidence * 100)}%</span>
                      </div>
                    )}
                  </div>
                  <div className="space-y-4">
                    ...
                    {result.aiExplanation && (
                      <div>
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">{result.aiExplanation}</p>
                      </div>
                    )}
                    <div>
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        <Input
                          ...
                          onChange={(e) => updateMarks(index, Math.min(parseInt(e.target.value) || 0, result.maxMarks))}
                          className="w-24"
                        />
                        <span className="text-sm text-muted-foreground">out of {result.maxMarks}</span>
                      </div>
                    </div>
                  </div>
                </div>
